...
(defmethod protocol-testcase-begin progn ((p protocol-trace) testcase)
  (format (slot-value p 'stream-output) "~&~@<PROTOCOL-TESTCASE-BEGIN ~S~@:>" testcase))

(defmethod protocol-testcase-end progn ((p protocol-trace) testcase outcome)
  (format (slot-value p 'stream-output) "~&~@<PROTOCOL-TESTCASE-END ~S ~S~@:>" testcase outcome))

(defmethod protocol-assertion-begin progn ((p protocol-trace) assertion arguments)
  (format (slot-value p 'stream-output) "~&~@<PROTOCOL-ASSERTION-BEGIN ~S ~{ ~S ~}~@:>" assertion arguments))

(defmethod protocol-assertion-end progn ((p protocol-trace) assertion outcome)
  (format (slot-value p 'stream-output) "~&~@<PROTOCOL-ASSERTION-END ~S ~S~@:>" assertion outcome))
...
when reporting the corresponding error condition.

When the first BODY form is the keyword :REPORT, then the second BODY form 
must be function of a STREAM.  This function is then used to generate an informational
message when reporting the corresponding error condition. This allows to add dynamic context
to this informational messsage, like the value of some variables."
  (let (description actual-body)
    (cond
      ((null body)
       (setf actual-body body))
      ((stringp (first body))
       (setf description (first body)
             actual-body (rest body)))
      ((eq :report (first body))
       (if (>= (length body) 2)
           (setf description
                 `(with-output-to-string (assertion-description)
                    (funcall ,(second body) assertion-description))
                 actual-body (rest (rest body)))
           (error "The :REPORT keyword requires a parameter.")))
      (t
       (setf description
             (with-output-to-string (assertion-description)
               ...
               (dolist (form body)
                 (format assertion-description "~&  ~S" form))
               (format assertion-description "~&~%It has no specific description."))
             actual-body body)))
     ...
     (let ((testsuite-p (eq *current-protocol* nil))
       ...
       (restart-case
         ...
         (skip ()
           :report
           (lambda (stream)
             (format stream "~@<Skip the rest of test case ~A and continue testing.~@:>" ,(symbol-name testcase-name)))
           (values)))
            ...
            (if (not (= (length vector1) (length vector2)))
                ...
                (loop :with difference-seen = nil
                      ...
                      :do (progn
                            ...
                            (format stream "~&  VECTOR1 and VECTOR2 differ at index ~A where they feature items ~S and ~S." index (aref vector1 index) (aref vector2 index))))))
